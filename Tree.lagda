...
\begin{code}
    ...
    --
      ...
      -- indMax*-≤L : ∀ {n t} {os : Vec Tree n} → t ≤ indMax* (o ∷ os)
      -- indMax*-≤L = indMax-≤L

      -- indMax*-≤R : ∀ {n t} {os : Vec Tree n} → indMax* os ≤ indMax* (o ∷ os)
      -- indMax*-≤R {o = t} = indMax-≤R {t1 = t}

      -- indMax*-≤-n : ∀ {n} {os : Vec Tree n} (f : Fin n) → lookup f os ≤ indMax* os
      -- indMax*-≤-n {os = t ∷ os} Fin.zero = indMax*-≤L {o = t} {os = os}
      -- indMax*-≤-n {os = t ∷ os} (Fin.suc f) = indMax*-≤-n f ≤⨟ (indMax*-≤R {o = t} {os = os})
